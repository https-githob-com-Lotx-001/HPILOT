...
class VCruiseHelper:
  ...
  def _update_v_cruise_non_pcm(self, CS, enabled, is_metric, speed_limit_changed, hpilot_variables):
    ...
    v_cruise_delta = v_cruise_delta * (hpilot_variables.custom_cruise_increase_long if long_press else hpilot_variables.custom_cruise_increase)
    if (long_press or hpilot_variables.custom_cruise_increase % 5 == 0) and self.v_cruise_kph % v_cruise_delta != 0:  # partial interval
