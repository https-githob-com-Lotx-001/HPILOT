...
from openpilot.selfdrive.hpilot.controls.hpilot_planner import HpilotPlanner
...
def hpilot_thread():
  ...
  sm = messaging.SubMaster(['carState', 'controlsState', 'deviceState', 'hpilotCarControl', 'hpilotNavigation',
                            'hpilotPlan', 'liveLocationKalman', 'longitudinalPlan', 'modelV2'], poll='modelV2')
    ...
    if started:
      if CP is None:
        with car.CarParams.from_bytes(params.get("CarParams", block=True)) as msg:
          CP = msg
          hpilot_planner = HpilotPlanner(CP)
          hpilot_planner.update_hpilot_params()

      if sm.updated['modelV2']:
        hpilot_planner.update(sm['carState'], sm['controlsState'], sm['hpilotCarControl'], sm['hpilotNavigation'],
                                  sm['liveLocationKalman'], sm['modelV2'])
        hpilot_planner.publish(sm, pm)
