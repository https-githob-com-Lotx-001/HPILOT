...
class HpilotPlanner:
  def __init__(self, CP):
    ...
    self.staging = get_short_branch() in ["Hpilot-Development", "Hpilot-Staging", "Hpilot-Testing"]
  ...
  def update(self, carState, controlsState, hpilotCarControl, hpilotNavigation, liveLocationKalman, modelData):
    ...
    if self.lead_one.status and self.CP.openpilotLongitudinalControl:
      ...
      self.jerk, self.t_follow = self.update_follow_values(base_jerk, self.lead_one, base_t_follow, hpilotCarControl.trafficModeActive, v_ego, v_lead)
    else:
    ...
    if self.conditional_experimental_mode and self.CP.openpilotLongitudinalControl or self.green_light_alert:
      self.cem.update(carState, controlsState.enabled, hpilotNavigation, self.lead_one, modelData, road_curvature, self.t_follow, v_ego)
  ...
  def publish(self, sm, pm):
    ...
    hpilotPlan.jerk = float(self.jerk)
