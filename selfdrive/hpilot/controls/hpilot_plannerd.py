...
from openpilot.selfdrive.hpilot.controls.lib.conditional_experimental_mode import ConditionalExperimentalMode
from openpilot.selfdrive.hpilot.controls.lib.hpilot_functions import CITY_SPEED_LIMIT, CRUISING_SPEED, calculate_lane_width, calculate_road_curvature
from openpilot.selfdrive.hpilot.controls.lib.map_turn_speed_controller import MapTurnSpeedController
...
class HpilotPlannerd:
  ...
  def update(self, carState, controlsState, hpilotCarControl, hpilotNavigation, liveLocationKalman, modelData, radarState):
    ...
    self.t_follow = self.update_t_follow(controlsState, hpilotCarControl, radarState, v_ego, v_lead)
    ...
    if self.conditional_experimental_mode and self.CP.openpilotLongitudinalControl or self.green_light_alert:
      self.cem.update(carState, controlsState.enabled, hpilotNavigation, modelData, radarState, road_curvature, self.t_follow, v_ego)

  def update_t_follow(self, controlsState, hpilotCarControl, radarState, v_ego, v_lead):
    if self.traffic_mode and hpilotCarControl.trafficModeActive:
    ...
    lead_distance = radarState.leadOne.dRel + (self.increased_stopping_distance if not hpilotCarControl.trafficModeActive else 0)

    # Offset by FrogAi for Hpilot for a more natural takeoff with a lead
    ...
    # Offset by FrogAi for Hpilot for a more natural approach to a slower lead
  ...
  def publish(self, sm, pm):
    hpilot_plan_send = messaging.new_message('hpilotPlan')
    hpilot_plan_send.valid = sm.all_checks(service_list=['carState', 'controlsState'])
    hpilotPlan = hpilot_plan_send.hpilotPlan

    hpilotPlan.adjustedCruise = float(min(self.mtsc_target, self.vtsc_target) * (CV.MS_TO_KPH if self.is_metric else CV.MS_TO_MPH))
    hpilotPlan.conditionalExperimental = self.cem.experimental_mode

    hpilotPlan.desiredFollowDistance = self.safe_obstacle_distance - self.stopped_equivalence_factor
    hpilotPlan.safeObstacleDistance = self.safe_obstacle_distance
    hpilotPlan.safeObstacleDistanceStock = self.safe_obstacle_distance_stock
    hpilotPlan.stoppedEquivalenceFactor = self.stopped_equivalence_factor

    hpilotPlan.laneWidthLeft = self.lane_width_left
    hpilotPlan.laneWidthRight = self.lane_width_right
    hpilotPlan.minAcceleration = self.min_accel
    hpilotPlan.maxAcceleration = self.max_accel
    hpilotPlan.tFollow = float(self.t_follow)
    hpilotPlan.vCruise = float(self.v_cruise)

    hpilotPlan.redLight = self.cem.red_light_detected

    hpilotPlan.slcOverridden = bool(self.override_slc)
    hpilotPlan.slcOverriddenSpeed = float(self.overridden_speed)
    hpilotPlan.slcSpeedLimit = self.slc_target
    hpilotPlan.slcSpeedLimitOffset = SpeedLimitController.offset
    hpilotPlan.unconfirmedSlcSpeedLimit = SpeedLimitController.desired_speed_limit

    hpilotPlan.vtscControllingCurve = bool(self.mtsc_target > self.vtsc_target)

    pm.send('hpilotPlan', hpilot_plan_send)
