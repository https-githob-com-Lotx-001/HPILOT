...
from openpilot.selfdrive.hpilot.controls.lib.model_manager import MODELS_PATH
...
class HpilotFunctions:
  ...
  @classmethod
  def backup_hpilot(cls):
    hpilot_backup_directory = "/data/backups"
    os.makedirs(hpilot_backup_directory, exist_ok=True)

    auto_backups = sorted(glob.glob(os.path.join(hpilot_backup_directory, "*_auto")),
    ...
    backup_path = os.path.join(hpilot_backup_directory, backup_folder_name)

    if not os.path.exists(backup_path):
      cmd = ['sudo', 'cp', '-a', f"{BASEDIR}", f"{backup_path}/"]
      cls.run_cmd(cmd, f"Successfully backed up Hpilot to {backup_folder_name}.", f"Failed to backup Hpilot to {backup_folder_name}.")
  ...
  def setup_hpilot(cls):
    ...
    hpilot_boot_logo = f'{BASEDIR}/selfdrive/hpilot/assets/other_images/hpilot_boot_logo.png'
  ...
  @classmethod
  def uninstall_hpilot(cls):
    if os.path.exists(MODELS_PATH):
      shutil.rmtree(MODELS_PATH)

    original_boot_logo = f'{BASEDIR}/selfdrive/hpilot/assets/other_images/bg.jpg'
    boot_logo_location = '/usr/comma/bg.jpg'
