...
from openpilot.selfdrive.hpilot.controls.lib.hpilot_functions import CITY_SPEED_LIMIT, CRUISING_SPEED, PROBABILITY, MovingAverageCalculator
...
class ConditionalExperimentalMode:
  ...
  def update(self, carState, enabled, hpilotNavigation, modelData, radarState, road_curvature, t_follow, v_ego):
    ...
    condition_met = self.check_conditions(carState, hpilotNavigation, lead, modelData, standstill, v_ego) and enabled
  ...
  def check_conditions(self, carState, hpilotNavigation, lead, modelData, standstill, v_ego):
    if standstill:
      self.status_value = 0
    ...
    if self.navigation and modelData.navEnabled and (hpilotNavigation.approachingIntersection or hpilotNavigation.approachingTurn) and (self.navigation_lead or not self.lead_detected):
      self.status_value = 7 if hpilotNavigation.approachingIntersection else 8
      return True
