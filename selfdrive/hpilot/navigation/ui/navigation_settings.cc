...
#include "selfdrive/hpilot/navigation/ui/navigation_functions.h"
#include "selfdrive/hpilot/navigation/ui/navigation_settings.h"

HpilotNavigationPanel::HpilotNavigationPanel(QWidget *parent) : QFrame(parent), scene(uiState()->scene) {
  ...
  HpilotListWidget *list = new HpilotListWidget(navigationWidget);
  ...
  QObject::connect(uiState(), &UIState::uiUpdate, this, &HpilotNavigationPanel::updateState);

  checkIfUpdateMissed();
}

void HpilotNavigationPanel::hideEvent(QHideEvent *event) {
  QWidget::hideEvent(event);
  mainLayout->setCurrentWidget(navigationWidget);
}

void HpilotNavigationPanel::updateState() {
  if (!isVisible() && downloadActive) updateVisibility(downloadActive);
  if (downloadActive) updateStatuses();
  if (schedule) downloadSchedule();
}

void HpilotNavigationPanel::updateStatuses() {
...
void HpilotNavigationPanel::updateVisibility(bool visibility) {
...
void HpilotNavigationPanel::checkIfUpdateMissed() {
...
void HpilotNavigationPanel::updateDownloadedLabel() {
...
void HpilotNavigationPanel::downloadSchedule() {
...
void HpilotNavigationPanel::cancelDownload(QWidget *parent) {
  if (HpilotConfirmationDialog::yesorno(tr("Are you sure you want to cancel the download?"), parent)) {
    std::thread([&] {
      std::system("pkill mapd");
    }).detach();
    if (HpilotConfirmationDialog::toggle(tr("Reboot required to re-enable map downloads"), tr("Reboot Now"), parent)) {
      Hardware::reboot();
    }
    downloadActive = false;
    updateVisibility(downloadActive);
...
void HpilotNavigationPanel::downloadMaps() {
...
void HpilotNavigationPanel::removeMaps(QWidget *parent) {
  if (HpilotConfirmationDialog::yesorno(tr("Are you sure you want to delete all of your downloaded maps?"), parent)) {
    std::thread([&] {
...
void HpilotNavigationPanel::setMaps() {
  std::thread([&] {
...
SelectMaps::SelectMaps(QWidget *parent) : QWidget(parent) {
  ...
  HpilotListWidget *statesList = new HpilotListWidget();
  ...
  HpilotListWidget *countriesList = new HpilotListWidget();
...
Primeless::Primeless(QWidget *parent) : QWidget(parent) {
  ...
  list = new HpilotListWidget(mainWidget);
...
void Primeless::updateStep() {
  currentStep = setupCompleted ? "../hpilot/navigation/navigation_training/setup_completed.png" :
                (mapboxPublicKeySet && mapboxSecretKeySet) ? "../hpilot/navigation/navigation_training/both_keys_set.png" :
                mapboxPublicKeySet ? "../hpilot/navigation/navigation_training/public_key_set.png" : "../hpilot/navigation/navigation_training/no_keys_set.png";
