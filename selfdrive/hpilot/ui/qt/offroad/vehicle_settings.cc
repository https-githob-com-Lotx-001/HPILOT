...
HpilotVehiclesPanel::HpilotVehiclesPanel(SettingsWindow *parent) : HpilotListWidget(parent) {
  ...
  std::vector<std::tuple<QString, QString, QString, QString>> vehicleToggles {
    ...
    {"LongitudinalTune", tr("Longitudinal Tune"), tr("Use a custom Toyota longitudinal tune.\n\nCydia = More focused on TSS-P vehicles but works for all Toyotas\n\nDragonPilot = Focused on TSS2 vehicles\n\nHpilot = Takes the best of both worlds with some personal tweaks focused around FrogsGoMoo's 2019 Lexus ES 350"), ""},
  ...
  for (const auto &[param, title, desc, icon] : vehicleToggles) {
    ...
    if (param == "LongitudinalTune") {
      std::vector<std::pair<QString, QString>> tuneOptions{
        ...
        {"FrogsGoMooTune", tr("Hpilot")},
      };
      toggle = new HpilotButtonsParamControl(param, title, desc, icon, tuneOptions);

      QObject::connect(static_cast<HpilotButtonsParamControl*>(toggle), &HpilotButtonsParamControl::buttonClicked, [this]() {
        if (started) {
          if (HpilotConfirmationDialog::toggle(tr("Reboot required to take effect."), tr("Reboot Now"), this)) {
            Hardware::reboot();
          }
  ...
  QObject::connect(uiState(), &UIState::uiUpdate, this, &HpilotVehiclesPanel::updateState);
...
void HpilotVehiclesPanel::updateState(const UIState &s) {
  if (!isVisible()) return;

  started = s.scene.started;
}

void HpilotVehiclesPanel::updateToggles() {
  std::thread([this]() {
...
void HpilotVehiclesPanel::updateCarToggles() {
...
void HpilotVehiclesPanel::setModels() {
  models = getCarNames(carMake.toLower());
  hideToggles();
}

void HpilotVehiclesPanel::hideToggles() {
