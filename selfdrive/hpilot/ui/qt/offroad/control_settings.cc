...
HpilotControlsPanel::HpilotControlsPanel(SettingsWindow *parent) : HpilotListWidget(parent) {
  ...
  for (const auto &[param, title, desc, icon] : controlToggles) {
    ...
    if (param == "AlwaysOnLateral") {
      ...
      toggle = new HpilotParamValueControl(param, title, desc, icon, steerRatioStock * 0.75, steerRatioStock * 1.25, std::map<int, QString>(), this, false, "", 1, 0.01);
...
void HpilotControlsPanel::hideEvent(QHideEvent *event) {
  hideToggles();
}
