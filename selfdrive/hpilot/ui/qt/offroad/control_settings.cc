...
HpilotControlsPanel::HpilotControlsPanel(SettingsWindow *parent) : HpilotListWidget(parent) {
  ...
  for (const auto &[param, title, desc, icon] : controlToggles) {
    ...
    if (param == "AlwaysOnLateral") {
      ...
      toggle = new HpilotParamValueToggleControl(param, title, desc, icon, 0, 99, std::map<int, QString>(), this, false, tr(" mph"), 1, 1, pauseLateralToggles, pauseLateralToggleNames);
...
void HpilotControlsPanel::updateMetric() {
  ...
  HpilotParamValueControl *customCruiseLongToggle = static_cast<HpilotParamValueControl*>(toggles["CustomCruiseLong"]);
